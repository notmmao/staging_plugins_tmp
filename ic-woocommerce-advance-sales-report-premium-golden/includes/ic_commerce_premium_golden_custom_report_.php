...
if ( ! class_exists( 'IC_Commerce_Premium_Golden_Detail_report' ) ) {
	...
	class IC_Commerce_Premium_Golden_Detail_report extends IC_Commerce_Premium_Golden_Fuctions{
                                ...
                                <div class="search_report_form">
                                    ...
                                    <form action="" name="Report" id="search_order_report" method="post">
                                        <div class="form-table">
                                            ...
                                            <?php  do_action("ic_commerce_detail_page_search_form_before_order_by", $this);?>
                                            ...
                                            <div class="form-group">
                                                <div class="FormRow" style="width:100%">
                                                    ...
                                                    <input type="hidden" name="order_item_name" 		id="order_item_name" 	value="<?php echo $order_item_name;?>" />
                                                    <input type="hidden" name="coupon_code" 			id="coupon_code" 		value="<?php echo $coupon_code;?>" />
                                                    <input type="hidden" name="coupon_codes" 			id="coupon_codes" 		value="<?php echo $coupon_codes;?>" />
                                                    ...
                                                    <input type="hidden" name="state_code" 				id="state_code" 		value="<?php echo $state_code;?>" />-->
                                                    ...
                                                    <input type="hidden" name="page"  					id="page" 				value="<?php echo $this->get_request('page',$page,true);?>" />
                                                    ...
                                                    <input type="hidden" name="purchased_product_id"  	id="purchased_product_id" value="-1" />
                                                   	<input type="hidden" name="do_action_type" 			id="do_action_type" 	value="<?php echo $this->get_request('do_action_type','detail_page',true);?>" />
                                                    ...
                                                    <input type="hidden" name="variation_id"  			id="variation_id" 		value="<?php echo $this->get_request('variation_id','-1',true);?>" />
                                                    <input type="hidden" name="variation_only" 			id="variation_only"		value="<?php echo $this->get_request('variation_only','-1',true);?>" />
                                                    <!--<input type="hidden" name="order_meta_key" 		id="order_meta_key"		value="<?php echo $this->get_request('order_meta_key','-1',true);?>" />-->
                                                    <input type="hidden" name="amount_greater_zero" 	id="amount_greater_zero" value="<?php echo $this->get_request('amount_greater_zero',$order_meta_key,true);?>" />
		...
		function ic_commerce_custom_report_normal($type = 'total_row', $columns = array()){
				$order_items 		= $this->ic_commerce_custom_report_normal_query($type, $columns);				
				if(count($order_items) > 0):
				
						$TotalOrderCount 	= 0;
						$TotalAmount 		= 0;
						$TotalShipping		= 0;
						$summary 			= $this->ic_commerce_custom_report_normal_query('total_row', $columns);
						...
						$columns			= apply_filters("ic_commerce_normal_view_grid_columns",$columns);
						$order_items		= apply_filters("ic_commerce_normal_view_data_grid",$order_items,$columns, $zero, $type);
						...
						$order_items		= apply_filters("ic_commerce_normal_view_data_grid_after_create_grid_items",$order_items,$columns, $zero, $type);
		...
		function ic_commerce_custom_report_detail($type = 'total_row', $columns = array()){
				global $wpdb;
			
				$order_items 		= $this->ic_commerce_custom_report_detail_query($type, $columns );
				if(count($order_items) > 0):
					
						$TotalOrderCount 	= 0;
						$TotalAmount 		= 0;
						$summary 			= $this->ic_commerce_custom_report_detail_query('total_row', $columns);
						$total_pages		= isset($summary['total_row_count']) ? $summary['total_row_count'] : 0;
						$columns 			= $this->grid_columns("details_view");
						...
						$columns			= apply_filters("ic_commerce_details_view_grid_columns",$columns);						
						$order_items		= apply_filters("ic_commerce_details_view_data_grid",$order_items,$columns, $zero, $type);
												
						$grid_object		= $this->get_grid_object();//Added 20150223
						$order_items		= $grid_object->create_grid_items($columns,$order_items);//Added 20150223
						
						$order_items		= apply_filters("ic_commerce_details_view_data_grid_after_create_grid_items",$order_items,$columns, $zero, $type);
						...
						<table style="width:100%" class="widefat widefat_detial_table">
							...
							foreach ( $order_items as $key => $order_item ) :
									$order_id		=  isset($order_item->order_id) ? $order_item->order_id : 0;
									$TotalAmount 	=  $TotalAmount + (isset($order_item->total_price) ? $order_item->total_price : 0);
									$zero_prize[$order_item->order_currency] = isset($zero_prize[$order_item->order_currency]) ? $zero_prize[$order_item->order_currency] : $this->price(0, array('currency' => $order_item->order_currency));
		...
		function ic_commerce_custom_report_normal_query($type = 'total_row', $columns = array()){
					global $wpdb;
					...
					if(!$this->normal_sql_query){
						...
						$sql = apply_filters("ic_commerce_normal_view_where_query", $sql, $request, $type, $page, $columns);
				...
				//if($type != 'total_row'){
					...
					$sql .= apply_filters("ic_commerce_normal_view_group_query", $group_sql, $request, $type, $page, $columns);
				//}
				...
				if($type == 'total_row'){
					if($this->all_row_result){
						...
						if($count_generated == 1 || ($p > 1)){
							...
							$grid_object		= $this->get_grid_object();//Added 20150223
							$order_items		= $grid_object->create_grid_items($total_columns,$order_items);//Added 20150223
							*/
							
							
							$order_items 	= apply_filters("ic_commerce_normal_view_data_items", $order_items, $request, $type, $page, $columns);
							$summary = $this->get_count_total($order_items,'order_total');
							$order_items = $summary;
		...
		function ic_commerce_custom_report_detail_query($type = 'total_row', $columns = array()){
				global $wpdb, $request;
				...
				if(!$this->detail_sql_query){
					...
					$columns_sql = apply_filters("ic_commerce_details_view_select_query", $columns_sql, $request, $type, $page, $columns);
					...
					if($variation_only  && $variation_only != "-1" && $variation_only == "1") {
						$sql .= " LEFT JOIN  {$wpdb->prefix}woocommerce_order_itemmeta 	as woocommerce_order_itemmeta_variation			ON woocommerce_order_itemmeta_variation.order_item_id 		= 	woocommerce_order_items.order_item_id";
					}
					...
					$sql = apply_filters("ic_commerce_details_view_join_query", $sql, $request, $type, $page, $columns);
					...
					$sql = apply_filters("ic_commerce_details_view_where_query", $sql, $request, $type, $page, $columns);
				...
				$sql .= apply_filters("ic_commerce_details_view_group_query", $group_sql, $request, $type, $page, $columns);
				//}
				...
				*/
				...
				if($type == 'total_row'){
					if($this->all_row_result){
						...
						if($count_generated == 1 || ($p > 1)){
							...
							$order_items = apply_filters("ic_commerce_details_view_data_items", $order_items, $request, $type, $page, $columns);
				...
				if($type == 'limit_row'){					
					$order_sql = " ORDER BY {$sort_by} {$order_by}";
					$sql .= apply_filters("ic_commerce_details_view_order_query", $order_sql, $request, $type, $page, $columns);
					$sql .= " LIMIT $start, $limit";
					$order_items = $wpdb->get_results($sql);
					if(strlen($wpdb->last_error) > 0){
						echo $wpdb->last_error;
		...
		function ic_commerce_custom_admin_report_ajax_request($type, $columns = array()) {
			...
			if (!empty( $_POST['action'] ) ) {
				...
				if($detail_view == "yes"){					
					$columns 	= $this->grid_columns("details_view");
					$this->ic_commerce_custom_report_detail($type, $columns );
				}else{
					$columns 	= $this->grid_columns("normal_view");
					$this->ic_commerce_custom_report_normal($type, $columns );
				}
			}else{
		...
		function ic_commerce_custom_report_page_export_csv(){
			...
			if($detail_view == "yes"){
				...
				$order_items = $this->ic_commerce_custom_report_detail_query('all_row', $columns);
				$summary 	= $this->ic_commerce_custom_report_detail_query('total_row', $columns);
			}else{
				$columns 	= $this->export_column("save_normal_column","normal_view");
				$order_items= $this->ic_commerce_custom_report_normal_query('all_row', $columns);
				$summary	= $this->ic_commerce_custom_report_normal_query('total_row', $columns);
			...
			$columns = apply_filters("ic_commerce_export_csv_columns_after_shipping_information", $columns);
			...
			foreach ( $order_items as $rkey => $rvalue ):
				...
				foreach($columns as $key => $value):
					switch ($key) {
						...
						case "item_discount":
							$td_value 				=  isset($rvalue->$key) ? $rvalue->$key : 0;
							$export_rows[$i][$key]	= $td_value > 0 ? $this->price($td_value, array('currency' => $order_item->order_currency)) : $zero_prize[$order_item->order_currency];
							break;
						case "order_date":

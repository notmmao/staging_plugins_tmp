...
class gapi
{
  ...
  public function requestReportData($report_id, $dimensions, $metrics, $sort_metric=null, $filter=null, $start_date=null, $end_date=null, $start_index=1, $max_results=30)
  {
    ...
    else 
    {
		...
		$this->request_report_data_error = strip_tags($response['body']);
		return false;
	  
    }
  }
  
  var $request_report_data_error = '';
  function get_request_report_error(){
		if(strlen($this->request_report_data_error) > 0){
			return str_replace("Error=","",$this->request_report_data_error);	
		}
		return '';
   }
  ...
  protected function authenticateUser($email, $password)
  {
    ...
    if(substr($response['code'],0,1) != '2' || !is_array($auth_token) || empty($auth_token['Auth']))
    {
	 	...
	 	//$this->authenticate_error = strip_tags($response['body']);		
		$this->authenticate_error = strip_tags($response['body']);
		return false;
    }
  ...
  var $authenticate_error = '';
  function authenticate(){
		if(strlen($this->authenticate_error) > 0){
			return str_replace("Error=","",$this->authenticate_error);	
		}
		return '';
   }

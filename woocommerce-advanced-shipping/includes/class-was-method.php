...
class WAS_Advanced_Shipping_Method extends WC_Shipping_Method {
	...
	public function __construct() {
		...
		$this->matched_methods	 	= $this->was_match_methods();
	 ...
	 */
	public function was_match_methods() {
		...
		foreach ( $methods as $method ) :
			...
			$match = $this->was_match_conditions( $condition_groups );
	 ...
	 */
	public function was_match_conditions( $condition_groups = array() ) {
		...
		foreach ( $condition_groups as $condition_group => $conditions ) :
			...
			foreach ( $conditions as $condition ) :
				...
				$match 		= apply_filters( 'was_match_condition_' . $condition['condition'], false, $condition['operator'], $condition['value'] );
	...
	public function hide_all_shipping_when_free_is_available( $available_methods ) {
		...
		foreach ( $available_methods as $key => $method ) :
			...
			if ( 0 == $method->cost ) :
				return array( $available_methods[ $key ] );
			endif;

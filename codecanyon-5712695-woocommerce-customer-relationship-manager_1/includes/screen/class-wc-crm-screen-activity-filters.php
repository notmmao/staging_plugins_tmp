...
class WC_CRM_Screen_Activity_Filters {
  ...
  public static function types_of_activity_filter()
  {
    $activity_types = array(
      'email' => __('Email', 'wc_crm'),
      'phone call' => __('Phone Call', 'wc_crm')
    );
      ?>
      <select name='activity_types' id='dropdown_activity_types'>
        <option value=""><?php _e( 'Show all types', 'woocommerce' ); ?></option>
        <?php
          foreach ( $activity_types as $type_key => $type_title ) {
            echo '<option value="' . esc_attr( $type_key ) . '"';

            if ( isset( $_REQUEST['activity_types'] ) ) {
              selected( $type_key, $_REQUEST['activity_types'] );
            }
            echo '>' . esc_html__( $type_title, 'wc_crm' ) .' </option>';
          }
        ?>
      </select>
      <?php

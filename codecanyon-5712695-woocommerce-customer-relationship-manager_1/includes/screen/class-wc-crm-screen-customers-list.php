      	...
      	if($group_data->group_type == 'static'){
			...
			return $ids; 
          	...
          	if(!empty($group_data->group_total_spent)){
				...
				$filter .= " AND customer.order_value $mark $spent ";
			...
			if( !empty($group_data->group_user_role) ){
				...
				if($group_user_role != 'any'){
					if($group_user_role == 'guest'){
						...
						$filter .= "AND customer.capabilities LIKE '%".$group_user_role."%'
						";
					}
				}
			}
			if( !empty($group_data->group_customer_status) ){
				...
				if(!empty($group_customer_status)){
					...
					$filter .= "AND customer.status IN( '". implode("', '", $group_customer_status) . "' )
					";
				}
			}
			...
			$d_from = false;
			if( !empty($group_data->group_last_order_from) &&  strtotime( $group_data->group_last_order_from ) !== false ){            
				$d_from = strtotime( $group_data->group_last_order_from );
			}
			$d_to = false;
			if( !empty($group_data->group_last_order_to) &&  strtotime( $group_data->group_last_order_to ) !== false ){            
			...
			if( $d_to || $d_from ){
				...
				switch ($mark) {
					case 'before':
					$filter .= "AND  DATE(customer.last_purchase) <= '".date( 'Y-m-d', $d_from ) . "' AND customer.last_purchase <> '0000-00-00 00:00:00'
					";
					break;
					case 'after':
					$filter .= "AND  DATE(customer.last_purchase) >= '".date( 'Y-m-d', $d_from ) . "' AND customer.last_purchase <> '0000-00-00 00:00:00'
					...
					break;
					case 'between':
					$filter .= "AND  DATE(customer.last_purchase) >= '".date( 'Y-m-d', $d_from ) . "' AND  DATE(customer.last_purchase) <= '".date( 'Y-m-d', $d_to ) . "'
					";
					break;
				}
			}

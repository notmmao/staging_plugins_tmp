...
class WC_CRM {
	...
	public function admin_enqueue_styles ( $hook = '' ) {
		...
		if( !in_array( $hook, $hooks) && $post_type != 'wc_crm_accounts'){
			return;
		}			
	...
	public function admin_enqueue_scripts ( $hook = '' ) {
		...
		switch ($screen) {
			...
			case 'phone_call':
				...
				$scripts[] = $this->_token . '-phone_call';
		...
		wp_enqueue_script( $this->_token . '-account', esc_url( $this->assets_url ) . 'js/account.js' );
		wp_enqueue_script( $this->_token . '-groups', esc_url( $this->assets_url ) . 'js/groups.js' );
		wp_enqueue_script( $this->_token . '-fonticonpicker', esc_url( $this->assets_url ) . 'plugins/fontpicker/jquery.fonticonpicker.js' );
		...
		wp_register_script( $this->_token . '-phone_call',esc_url( $this->assets_url ) . 'js/phone_call.js' );
		...
		wp_register_script( $this->_token . '-admin', esc_url( $this->assets_url ) . 'js/admin' . $this->script_suffix . '.js', $scripts, $this->_version );

...
CREATE TABLE {$wpdb->prefix}wc_crm_statuses (
	...
	public static function plugin_action_links( $links ) {
	    ...
	    $plugin_links['settings'] = sprintf( '<a href="%s">%s</a>', '' . admin_url( 'admin.php?page=' . WC_CRM_TOKEN . '-settings' ) . '', __( 'Settings', 'wc_crm' ) );
	...
	*/
	...
	if ( $file == plugin_basename( WC_CRM_BASENAME ) ) {
	  $row_meta = array(
	    'docs'    => '<a href="' . esc_url( apply_filters( 'wc_crm_docs_url', 'http://actualityextensions.com/documentation/woocommerce-customer-relationship-manager/' ) ) . '" title="' . esc_attr( __( 'View POS Documentation', 'wc_crm' ) ) . '">' . __( 'Docs', 'wc_crm' ) . '</a>',
	    'support' => '<a href="' . esc_url( apply_filters( 'wc_crm_docs_url', 'https://actualityextensions.zendesk.com/hc/en-us/' ) ) . '" title="' . esc_attr( __( 'Visit Support Help Desk', 'wc_crm' ) ) . '">' . __( 'Support', 'wc_crm' ) . '</a>',
	  );

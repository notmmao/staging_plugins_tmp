...
class WC_CRM_Customer {
	...
	public function __get( $key ) {
		...
		$value = isset($this->user_data->$key) ? $this->user_data->$key : '';

		switch ($key) {
			case 'name':
				if( $this->options->customer_name == 'lf' ){
					$n = array();
	...
	public function __construct($customer_id = 0) {
		...
		$this->user_data   = $wpdb->get_row("SELECT * FROM {$wpdb->prefix}wc_crm_customer_list WHERE c_id = {$this->customer_id} LIMIT 1");
		if($this->user_data){
			$this->user_id     = $this->user_data->user_id;
			$this->order_id    = $this->user_data->order_id;
		}else{
			$this->user_id     = 0;
			$this->order_id    = 0;
		}

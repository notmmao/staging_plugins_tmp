...
class WC_Gateway_Authorize_Net_CIM_Credit_Card extends WC_Gateway_Authorize_Net_CIM {
	...
	public function __construct() {
		...
		parent::__construct(
			...
			WC_Authorize_Net_CIM::TEXT_DOMAIN,
			array(
				'method_title'       => __( 'Authorize.net CIM', WC_Authorize_Net_CIM::TEXT_DOMAIN ),
				'method_description' => __( 'Allow customers to securely pay using their credit cards with Authorize.net CIM.', WC_Authorize_Net_CIM::TEXT_DOMAIN ),
				'supports'           => array(
				...
				'environments'       => array( 'production' => __( 'Production', WC_Authorize_Net_CIM::TEXT_DOMAIN ), 'test' => __( 'Test', WC_Authorize_Net_CIM::TEXT_DOMAIN ) ),
	...
	protected function get_order_for_refund( $order_id, $amount, $reason ) {
		...
		if ( $this->get_order_meta( $order->id, 'payment_token' ) ) {
			...
			if ( empty( $order->refund->customer_profile_id ) ) {
				$error_message = __( 'Order is missing customer profile ID.', WC_Authorize_Net_CIM::TEXT_DOMAIN );
			}
			...
			if ( empty( $order->refund->last_four ) || empty( $order->refund->expiry_date ) ) {
				...
				$error_message = __( 'Order is missing the last four digits or expiration date of the credit card used.', WC_Authorize_Net_CIM::TEXT_DOMAIN );
			}
		}

		if ( ! empty( $error_message ) ) {
			return new WP_Error( 'wc_' . $this->get_id() . '_refund_error', __( '%s Refund error - %s', WC_Authorize_Net_CIM::TEXT_DOMAIN ), $this->get_method_title(), $error_message );
		}

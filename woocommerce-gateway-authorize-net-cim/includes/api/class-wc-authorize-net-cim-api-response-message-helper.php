...
class WC_Authorize_Net_CIM_API_Response_Message_Handler extends SV_WC_Payment_Gateway_API_Response_Message_Helper {
	...
	public function __construct( $response ) {
		...
		parent::__construct( WC_Authorize_Net_CIM::TEXT_DOMAIN );
	}
	...
	public function get_user_message( $message_id ) {
		...
		switch ( $message_id ) {
			...
			case 'authorize_net_error_try_later':
				$message = __( 'Oops, sorry! A temporary error occurred. Please try again in 5 minutes.', WC_Authorize_Net_CIM::TEXT_DOMAIN );
			break;

			case 'authorize_net_authorized_but_not_settled':
				$message = __( 'This transaction was authorized successfully, but could not be settled. Please contact us.', WC_Authorize_Net_CIM::TEXT_DOMAIN );
			break;

			case 'authorize_net_echeck_mismatch':
				$message = __( 'The name and/or bank account type does not match. Please re-enter and try again.', WC_Authorize_Net_CIM::TEXT_DOMAIN );
			break;

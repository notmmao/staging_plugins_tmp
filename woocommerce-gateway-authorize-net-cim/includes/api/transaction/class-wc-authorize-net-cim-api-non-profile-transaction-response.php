	...
	public function get_transaction_status_message() {
		...
		if ( isset( $this->response->transactionResponse->messages->message ) ) {
			...
			foreach ( $this->response->transactionResponse->messages->message as $message ) {
				...
				$messages[] = sprintf( __( 'Message Code: %s - %s', WC_Authorize_Net_CIM::TEXT_DOMAIN ), (string) $message->code, (string) $message->description );
			}
		}
		...
		if ( isset( $this->response->transactionResponse->errors->error ) ) {
			...
			foreach ( $this->response->transactionResponse->errors->error as $error ) {
				...
				$messages[] = sprintf( __( 'Error Code: %s - %s', WC_Authorize_Net_CIM::TEXT_DOMAIN ), (string) $error->errorCode, (string) $error->errorText );
			}
		}

...
class WC_Authorize_Net_CIM_API_Profile_Response extends WC_Authorize_Net_CIM_API_Transaction_Response {
	...
	protected function parse_direct_response() {
		...
		if ( empty( $response ) ) {
			throw new SV_WC_Payment_Gateway_Exception( __( 'Could not parse direct response.', WC_Authorize_Net_CIM::TEXT_DOMAIN ) );
		}
	...
	public function get_transaction_status_message() {
		...
		return sprintf( __( 'Code: %s - %s', WC_Authorize_Net_CIM::TEXT_DOMAIN ), $this->get_transaction_response_reason_code(), $this->get_transaction_response_reason_text() );
	}

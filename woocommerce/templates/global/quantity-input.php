...
<?php if ($fixed_step != ''): ?>
    <select class="mod-quantity-input" name="<?php echo esc_attr( $input_name ); ?>" value="<?php echo esc_attr( $input_value ); ?>" >
        <?php foreach ($step_array = explode(',', $fixed_step) as $current_step): ?>
        <option value="<?php echo intval($current_step); ?>" <?php if($current_step == $input_value) echo 'selected'; ?>><?php echo $current_step ?></option>
        <?php endforeach ?>
    </select> 
<?php else: ?>
    <div class="quantity">

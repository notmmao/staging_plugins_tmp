...
class Product_Addon_Display {
	...
	public function totals( $post_id ) {
		...
		//$product_addons1 = get_product_addons($post_id );
		//foreach($product_addons1 as $result)
		//{
		       // print_r($result['options']);
			   //for()
			    
				// $newArray = array_merge($result['options']);
				
				
		  //}
		// $product_addons1 = get_product_addons($post_id );
		//foreach($product_addons1 as $result => $value)
        // {
		    // echo '<pre>';
           //		print_r($value['options'][0]);
			//echo $value['options'][0];

...
class Product_Addon_Cart {
	...
	public function add_cart_item($cart_item) 
	{
		...
		if (!empty( $cart_item['addons'] ) && apply_filters( 'woocommerce_product_addons_adjust_price', true, $cart_item)) {
			...
			if (isset($cart_item['pricing_item_meta_data']['_price']) && $cart_item['pricing_item_meta_data']['_price'] > 0) {
				$base_price = $cart_item['pricing_item_meta_data']['_price'];
				$cart_item['data']->set_price($cart_item['pricing_item_meta_data']['_price']);
			} else {
				$base_price = $cart_item['data']->get_regular_price();
			}

			// NOTE(stas): re-calcuate addon prices if 'size' add-on present;
			// All addon _PERCENTAGE_ prices must be calculated based on (product price + size addon % price) 

			// In case of measurement product the base price will be for sq.ft.

			$hasSizeOption = false;
			$modifiedBasePrice = $base_price;

			foreach ($cart_item['addons'] as $addon) {
				$normalizedAddonName = strtolower($addon['name']);

				if (strstr($normalizedAddonName, 'size')) {

					if (strpos($addon['price'], '%')) {
						$modifiedBasePrice = $base_price/100 * $addon['price'] + $base_price;
					} else {
						$modifiedBasePrice = $addon['price'] + $base_price;
					}

					// echo "Size option found and modified base price is: {$modifiedBasePrice}<br>";
			...
			foreach ( $cart_item['addons'] as $addon ) {
				if ($addon['price'] != 0) {
					...
					if (strstr($normalizedAddonName, 'size')) {
						$isSize = true;
					} else if (strstr($normalizedAddonName, 'mailing services')) {
						$isMailingFee = true;
					}

					// echo "Addon: {$normalizedAddonName}: ";

					if (strpos($addon['price'], '%')) {
						if(!$isSize) {
							// echo "(regular)";
							$addon['price'] = $modifiedBasePrice/100 * $addon['price'];
						} else {
							// echo "(size modifier)";
							$addon['price'] = $base_price/100 * $addon['price'];
						}
						$thisAddonPrice = $addon['price'];
					} else {
						$thisAddonPrice = $addon['price'];
					}
					...
					if ($isMailingFee) {
						// echo "(mailing)";
						$addon['price'] = $addon['price'] / $cart_item['quantity'];
						$thisAddonPrice = $addon['price'];
					}		
					
					// echo $thisAddonPrice;

					$extra_cost += $thisAddonPrice;
					// echo ", total:" . $extra_cost . "<br>";
				}

			}
			// echo "Total unit price: " . ($modifiedBasePrice + $extra_cost);
		...
		if ( ! empty( $cart_item['addons'] ) ) {
			foreach ( $cart_item['addons'] as $addon ) {
				...
				if ( $addon['price'] != 0 && apply_filters( 'woocommerce_addons_add_price_to_name', '__return_true' ) ) {
					if (strpos($addon['price'], '%')) {
						$addon['price'] = str_replace('%', '', $addon['price']);
						$name .= ' (' . woocommerce_price( $base_price/100 * $addon['price'] ) . ')';

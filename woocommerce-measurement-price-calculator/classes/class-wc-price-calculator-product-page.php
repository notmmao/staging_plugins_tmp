...
class WC_Price_Calculator_Product_Page {
	...
	private function remove_price_html_filters() {
		...
		remove_filter( 'woocommerce_get_price_html',           array( $this, 'price_per_unit_html' ), 10, 2 );
		remove_filter( 'woocommerce_get_variation_price_html', array( $this, 'price_per_unit_html' ), 10, 2 );
	...
	public function get_variation_regular_price( $price, $product, $min_or_max, $display ) {
		...
		if ( $display ) {
			...
			$price = round( WC_Price_Calculator_Compatibility::get_product_display_price( $product, $product->$variation_regular_price ), 2 );
		}
	...
	public function get_variation_sale_price( $price, $product, $min_or_max, $display ) {
		...
		if ( $display ) {
			...
			$price = round( WC_Price_Calculator_Compatibility::get_product_display_price( $product, $product->$variation_sale_price ), 2 );
		}
	...
	public function get_variation_price( $price, $product, $min_or_max, $display ) {
		...
		if ( $display ) {
			...
			$price = round( WC_Price_Calculator_Compatibility::get_product_display_price( $product, $product->$variation_price ), 2 );
		}
	...
	public function available_variation( $variation_data, $product, $variation ) {
		...
		$variation_data['price'] = WC_Price_Calculator_Compatibility::get_product_display_price( $variation );
	...
	public function enqueue_frontend_scripts() {
		...
		$wc_price_calculator_params = array(
			...
			'minimum_price'                  => $product->wc_measurement_price_calculator_min_price ? WC_Price_Calculator_Compatibility::get_product_display_price( $product, $product->wc_measurement_price_calculator_min_price ) : '',
			'measurement_type'               => $settings->get_calculator_type(),
		);

		// information required for either pricing or quantity calculator to function
		$wc_price_calculator_params['product_price'] = $product->is_type( 'variable' ) ? '' : WC_Price_Calculator_Compatibility::get_product_display_price( $product );
